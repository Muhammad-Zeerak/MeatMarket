...
class OrderService extends CoreService implements OrderServiceInterface
{
    ...
    {
        // \Log::info('Fetched create:', ['order' => $data]);
...
//                if (!$shop->open || $isClosed || ($time < $from)) {
    ...
    private function calculateOrder(Order $order, ?Shop $shop, array $data): void
    {
        ...
        $redeemPoints = data_get($data, 'redeemPoints', 0);
        if ($redeemPoints > 0) {
            $pointsValue = $this->convertPointsToValue($redeemPoints);
            $totalPrice = max($totalPrice - $pointsValue, 0);

            // now as the points are redeem we need to deduct from user end
            $user = User::find(data_get($data, 'user_id'));
            $user->points_balance -= $redeemPoints;
            $user->save();
        }
     ...
     * You need to implement this method based on your points system.
     *
     * @param int $points
     * @return float
     */
    private function convertPointsToValue(int $points): float
    {
