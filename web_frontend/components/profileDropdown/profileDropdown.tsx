...
export default function ProfileDropdown({ data }: Props) {
  ...
  const calculateCurrencyValue = (points: number | undefined) => {
    if (!points) return 0;
        ...
        <div className={cls.wrapper}>
          ...
          <div className={cls.body}>
            ...
            <div className={cls.flex}>
              <div className={cls.item}>
                <Wallet3LineIcon />
                <span className={cls.text}>{t("redeem.points")}:</span>
                <span className={cls.bold}>
                {(data.points_balance ?? 0)} ~ {calculateCurrencyValue(data.points_balance ?? 0)}
                </span>
              </div>
            </div>
