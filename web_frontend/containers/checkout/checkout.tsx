...
export default function CheckoutContainer({ data, children }: Props) {
  ...
  const { paymentType, paymentTypes } = useMemo(() => {
    ...
    // add this just in case the setting does not work
  ...
  const formik = useFormik({
    ...
    onSubmit: (values: OrderFormValues) => {
      ...
      const payload = {
        ...
        redeemPoints: formik.values.redeemPoints,
      };
      mutate(payload);
    },
  ...
  const { isLoading, mutate } = useMutation({
    mutationFn: (data: any) => {
      // console.log('Payload being sent to create order:', data); // Log the data
    ...
    onSuccess: async (data) => {
      ...
      if (formik.values.payment_type?.tag === "tap") {
        setPaymentLoading(true);
        try {
          const response = await tapPay({ order_id: payload.id });
          const redirectUrl = response.data?.original?.transaction?.url;
  ...
  const { isLoading: isLoadingTapPay, mutateAsync: tapPay } = useMutation({
    mutationFn: (data: any) => paymentService.tapPay(data),
  });
